...
namespace SDL {
    ...
    SDL_Point Renderer::get_output_size() const {
        ...
        if (result != 0) throw std::runtime_error{"cannot get renderer output size"};
        return {width, height};
    }
    ...
    Texture Renderer::create_texture_from_surface(Surface const & surface) const {
        ...
        if (texture == nullptr) throw std::runtime_error{"cannot create texture from surface"};
        return {texture};
    }

    void Renderer::fill_rect(SDL_Rect const & rect) const {
        const int result = SDL_RenderFillRect(self.get(), &rect);
        if (result != 0) throw std::runtime_error{"cannot render filled rectangle"};
    }
    ...
    void Renderer::set_viewport(const std::optional<SDL_Rect> & bounds) const {
        ...
        if (result != 0) throw std::runtime_error{"cannot set renderer viewport"};
    }
    ...
    void Renderer::set_render_draw_color(uint8_t red, uint8_t green, uint8_t blue,
        ...
        if (result != 0) throw std::runtime_error{"cannot set render draw color"};
    }
    ...
    void Renderer::render_copy(SDL::Texture const & texture, std::optional<SDL_Rect> const & src,
        ...
        if (result != 0) throw std::runtime_error{"cannot copy texture onto renderer buffer"};
    }
