...
namespace TTF {
    Font::Font(const std::string_view path, const int ptsize)
        ...
        if (self == nullptr) throw std::runtime_error{"failed to initialize font: "s + path.data()};
    }
    ...
    SDL::Surface Font::render_text_blended(const std::string_view text, const SDL_Color fg) const {
        ...
        if (surface == nullptr) throw std::runtime_error{"cannot render blended text onto surface"};
        return {surface};
    }
} // namespace TTF
